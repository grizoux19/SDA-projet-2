...
char* tmpp(ScrabbleDict*, char*, char*, int, int, int, int, char*);
char* tmpp(ScrabbleDict* sd , char* word, char* tmp, int start, int end, int index, int r, char* result)
{
    for(int p = 0; p <= end; p++)
    {
        checkk(sd, word, tmp, start, index, p, r, result);
        return result;
    }
}
...
char* getallcombinaison(ScrabbleDict* sd, char* word, int length, int r)
{
    ...
    {
        tmpp(sd, word, tmp, k + 1, length - 1, 0, r, result);
        //fprintf(stderr,"Je print le result : %s \n", result);
        return result;
    }
}
